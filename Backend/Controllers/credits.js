...
import employee from "../models/employee_role_type.js";
...
class CreditLeave{
    ...
    static async runYearlyCreditInsertion(req, res){                  
        try{             
            ...
            const employee_record = await employee.getRoleByIdEmployee(ROLE_TYPE_ID.employee);              
            ...
            const insert_yearly_credit = await leaveCredit.insertYearlyCredit(employee_data);             
    ...
    static async getAllEmployeeCredits(req, res){
        const leave_credit_record = await leaveCredit.getAllEmployeeCredit();
        return res.json({ success: true, result: leave_credit_record.result });
    }    
}

export default CreditLeave;
