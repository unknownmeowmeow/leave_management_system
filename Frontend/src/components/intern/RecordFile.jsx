...
=======
    useEffect(() => {
        axios
            .get("http://localhost:5000/api/leave/leave_transaction", { withCredentials: true })
            .then((res) => {
                if (res.data.success) {
    ...
    const handleTimeIn = async () => {
        try {
            ...
            alert(error.response?.data?.message || "Time In failed.");
        }
    };
    ...
    const handleTimeOut = async () => {
        try {
            ...
            alert(error.response?.data?.message || "Time Out failed.");
        }
    };
