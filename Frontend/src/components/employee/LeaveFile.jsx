...
export default function EmployeeApply() {
  ...
  const [leaveCredit, setLeaveCredit] = useState(null);
  ...
  // Fetch leave types and leave credit on load
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (resCredit.data.success)
          setLeaveCredit(resCredit.data.latest_credit);
      } else {
        setError(
          typeof res.data.message === "string"
            ? res.data.message
    ...
    <div style={containerStyle}>
      ...
      {leaveCredit !== null && (
        <div style={{ marginBottom: "15px", color: "blue", fontWeight: "bold" }}>
          Available Leave Credit: {typeof leaveCredit === 'number' ? leaveCredit : leaveCredit.credit} days
        </div>
      )}
      ...
      <form onSubmit={handleSubmit}>
        ...
        {/* Show notice day note if negative */}
